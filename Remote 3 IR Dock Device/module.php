...
class Remote3IRDockDevice extends IPSModuleStrict
{
    ...
    /**
     * Sends a command payload to the parent splitter (Remote Dock 3 Manager) via SendDataToParent.
     ...
     *  {"url":"http://<ip>:3777/hook/unfoldedcircle_dock3/<iid>/download?file=...&token=..."}
     */
    ...
    public function ExportDeviceDefinition(): string
    {
        ...
        $dir = IPS_GetKernelDir() . 'media/UCD3Exports/';
        if (!is_dir($dir)) {
            @mkdir($dir, 0775, true);
        }

        $fileName = 'UCD3_IRDevice_' . $this->InstanceID . '_' . date('Ymd_His') . '.json';
        $file = $dir . $fileName;
        ...
        if ($fullUrl !== '') {
            ...
            $this->UpdateFormField('ExportDownloadLink', 'caption', 'Export download link: (not available yet - Dock Manager download webhook missing)');
        }

        $relativeUrl = '/media/UCD3Exports/' . $fileName;

        $msg = "✅ Export saved:\n" .
            $file . "\n\n" .
            (($fullUrl !== '') ? ("Download URL (full):\n" . $fullUrl . "\n\n") : "") .
            "Download URL (relative):\n" .
            $relativeUrl . "\n\n" .
            "Note: The full download URL requires a download webhook in the Dock Manager/Splitter.";
    ...
    private function BuildCommandScriptContent(string $commandName): string
    {
        ...
        return "<?php\n\n" .
            "// Auto-generated by Remote 3 IR Dock Device (#{$instanceId})\n" .
